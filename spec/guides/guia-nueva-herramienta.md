# Guía — cómo crear una herramienta nueva en BSTools

> Redistribuido desde `SPEC.md` §2 ("Archivos de la carpeta"), §6 ("Plantillas") y §7 ("Cómo
> probar"). Texto literal del dueño, sin reescribir. Para las reglas no-negociables que gobiernan
> esta guía, ver [`spec/constitution/principles.md`](../constitution/principles.md); para el
> contrato técnico con Windows, [`spec/constitution/tech-stack.md`](../constitution/tech-stack.md).

## Archivos de la carpeta

Toda herramienta nueva vive en `apps/<NombreHerramienta>/`, nunca suelta en la
raíz del repositorio: el estándar de la casa solo permite un conjunto cerrado
de carpetas en la raíz (`apps`, `packages`, `src`, ...) y una carpeta suelta
ahí la reporta el linter de layout como fuera de sitio.
...
**Versión.** Cada carpeta lleva su propia versión semántica, declarada en el
mensaje del commit que cierra el trabajo (`NOMBRE 1.0.0: qué cambió`) — el commit
es el changelog versionado de este repo — y, cuando aplica, en el propio
`README.md` de la herramienta. No hay versión global del repositorio ni un
archivo de versiones aparte.
