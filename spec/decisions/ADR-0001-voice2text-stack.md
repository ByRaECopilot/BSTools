...
| **D6** ⏳ | Enlaces con **`yt-dlp`**, solo **HTTP/HTTPS**, solo contenido público, **sin cookies ni credenciales jamás**. No es alcance: es la postura legal (§11). **Aviso:** el dueño ha autorizado el uso de sus cookies y hay un **[ADR-0003](ADR-0003-voice2text-cookies-del-navegador.md) en `propuesto`** que **matizaría** esta decisión (nunca contraseñas, nunca en archivos locales, apagado por defecto) y sustituiría §11. **Hasta que se acepte, D6 rige tal cual**, y puede archivarse sin cambiar nada si el diagnóstico en curso dice que las cookies no hacían falta. |
